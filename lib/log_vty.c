...
DEFPY_NOSH(debug_uid_backtrace, debug_uid_backtrace_cmd,
	   "[no] debug unique-id UID backtrace",
	   NO_STR DEBUG_STR "Options per individual log message, by unique ID\n"
...
DEFUN(config_log_dmn_file, config_log_dmn_file_cmd,
      "log daemon " DAEMONS_LIST
      " file FILENAME [<emergencies|alerts|critical|errors|warnings|notifications|informational|debugging>$levelarg]",
      "Logging control\n"
...
DEFUN(no_config_log_dmn_file, no_config_log_dmn_file_cmd,
...
DEFPY(config_log_ec, config_log_ec_cmd, "[no] log error-category",
      NO_STR "Logging control\n"
...
DEFPY(config_log_xid, config_log_xid_cmd, "[no] log unique-id",
      NO_STR "Logging control\n"
...
DEFPY(log_immediate_mode, log_immediate_mode_cmd, "[no] log immediate-mode",
      NO_STR "Logging control\n"
